...
int sock_max, accept_sock, null_sock;
...
int read_write(int read_fd, int write_fd)
{
  ...
  for (i=0; i <MAX_CONN; i++) {
    if (sock_pair.uart[i] == -1 || sock_pair.client[i] == -1) continue;
    write_fd = sock_pair.uart[i];
...
int accept_connect()
{
  ...
  /* other end os /dev/null */
  client_sock = null_sock;
