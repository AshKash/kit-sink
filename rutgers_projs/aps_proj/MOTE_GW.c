 ...
 *
 * 03/19/2002: Changes for APS:
 *      All non REPLY packets are dropped
 *
 */
...
/*
 * MOTE_GW_INIT: initialize lower components. initialize component state, 
 * including constant portion of msgs. 
 */
char            TOS_COMMAND(MOTE_GW_INIT) () {
    CLR_GREEN_LED_PIN();
    TOS_CALL_COMMAND(MOTE_GW_SUB_INIT) ();	/* initialize lower
						 * components */
    TOS_CALL_COMMAND(MOTE_GW_SUB_UART_INIT) ();	/* initialize lower
						 * components */
    VAR(msg) = &VAR(data);
...
TOS_MsgPtr      TOS_EVENT(MOTE_GW_SUB_UART_RX_PACKET) (TOS_MsgPtr data) {
	...
	/*
	 * // if (TOS_CALL_COMMAND(UART_PACKET_SUB_TX_BYTES)(FLOW_STOP) == 
	 * 0) // goto ERROR_RFM_TX; 
	 */
	...
	// Some sort of error has occured
      ERROR_RFM_TX:
	/*
	 * tmp = data; // The error pkt must be filled here tmp->addr =
	 * 0x7e; tmp->group = LOCAL_GROUP; tmp->type = 0xff; // The flow
	 * control error handler
	 * 
	 * 
	 * // Failed printf("MOTE_GW: Error while sending to RFM\n"); 
	 */
	CLR_YELLOW_LED_PIN();
	// TOS_CALL_COMMAND(UART_PACKET_SUB_TX_BYTES)(FLOW_STOP);
	/*
	 * // Try to do flow control // send a special packet back to the
	 * uart // saying sender must slow down. failure is not checked
 ...
 */
char            TOS_EVENT(MOTE_GW_SUB_UART_TX_PACKET_DONE) (TOS_MsgPtr
							    data) {
    if (VAR(msg) == data) {
