...
/*
 * Turn a double into a fixpoint number int two's complement. Negative
 * numbers become: ~a + 1 
 */
#ifdef FULLPC
fixpoint
...
fp_int_fix(int x)
{
    ...
    if ((x < fp_integer(fp_min())) || (x >= fp_integer(fp_max())))
#ifdef FULLPC
	printf("sorry, %d doesn't fit in (%d hi, %d lo) bit fix point.\n",
	       x, HIBITS, LOBITS);
#endif
 ...
 * negative numbers it does floor(abs(x/y)), so we need to case on sign of 
 * x to fix that.
 * 
...
#ifdef FULLPC
fixpoint
fp_floor_div(fixpoint x, fixpoint y)
{
    fixpoint        answer;
...
fixpoint
fp_multiply(fixpoint x, fixpoint y)
{
    fixpoint        answer;
    unsigned int    xhi,
                    xlo,
	...
	if (fp_print_error)
		...
		("ERROR: fp_multiply() xhi*yhi = 0x%08x overflows by 0x%08x\n",
		 tmp, (tmp & (((int) OVERFLOWMASK) >> LOBITS)));
#endif
	fp_error = -1;
	abort();
	return (0);
...
fixpoint
fp_div_fractionpart(fixpoint remainder, fixpoint y)
{
    fixpoint        ten = fp_int_fix(10),
                    tmp,
                    answer = 0;
    int             ret = 0,
                    i;
