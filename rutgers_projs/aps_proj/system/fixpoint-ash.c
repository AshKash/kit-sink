 ...
 * Changes: Ashwin: Converted ints to int16 and unsigned int to uint16
 * which are defined in fixpoint.h TODO: Make fixpoint to be byteorder
 * independent. algo: All fixpoints are returned in n/w byteorder,
 * similaryl assume all fixpoint args are in n/w byteorder  
 */  
#ifdef FULLPC
#include <stdio.h>
#include <math.h>
...
#include "fixpoint-ash.h"
    
#define LOBITS 4
#define HIBITS 12
#define LOMASK       (~(0xffff << LOBITS))
#define HIMASK       ((~(0xffff << HIBITS)) << LOBITS)
    
#define SIGNBIT      (int16)(1 << (HIBITS+LOBITS-1))
#define OVERFLOWMASK (int16)(SIGNBIT | ~(HIMASK | LOMASK))
...
#define TwosComplement(_x_)  (-(_x_))
    int16 fp_verbose;
int16 fp_error = 0;		/* will contain error indicators for
				 * fp_multiply */
int16 fp_print_error = 1;	/* if 1, print overflow errors */
    ...
    fixpoint fp_fix(double x) 
{
    ...
    i = floor(x);
    fraction = x - i;
    p = (((int16) i) << LOBITS) | ((int16) ((x - i) * (1 << LOBITS)));
    if (negative)
    ...
    fixpoint fp_multiply(fixpoint x, fixpoint y) 
{
	 ...
	 */ 
	tmp = (xhi * yhi);
    if ((tmp & (((int16) OVERFLOWMASK) >> LOBITS)) != 0) {
	
#ifdef FULLPC
	    if (fp_print_error)
	    printf
		("ERROR: fp_multiply() xhi*yhi = 0x%08x overflows by 0x%08x\n",
		 tmp, (tmp & (((int16) OVERFLOWMASK) >> LOBITS)));
...
#ifdef FULLPC
	    printf("Error in fp_div_fractionpart: i = %d\n", i);
...
#ifdef FULLPC
	    printf("Error: fp_div -- can't divide by 0!\n");
