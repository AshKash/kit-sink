/* does not use fixpoint library
 * BUGS: does not do endian conversion */
#include "../system/include/aps_msgs-fp.h"
...
got_reply(TOS_Msg * am_pkt)
{
    ...
    case REP_SET_LM:{
	    float       *fp = (float *) (qmptr->data + 1);
	    printf("REP_SET_LM %d %f %f", (int) *qmptr->data,
		   *fp++, *fp++);
	}
	break;

    case REP_GET_COORD:{
	    float       *fp = (float *) qmptr->data;
	    printf("REP_GET_COORD %f %f", *fp++, *fp++);
	}
	break;
    ...
    case REP_DUMP_LMS:{
		...
		printf("(%f, %f, %d)\t", lmsptr->x, lmsptr->y, lmsptr->hops);
		lmsptr++;
	    }
	}
	...
	printf("REP_GET_CORRECTION %f",*(float *) qmptr->data);
	break;
...
read_cl(TOS_Msg * am)
{
	...
	items = sscanf(buf + n, "%d %f %f", &setlm, &fpx, &fpy);
	if (items != 3) {
	...
	((float *) (qmptr->data + 1))[0] = fpx;
	((float *) (qmptr->data + 1))[1] = fpy;
