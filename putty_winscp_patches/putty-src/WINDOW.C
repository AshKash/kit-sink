...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
	...
	for (j = 0; j < lenof(popup_menus); j++) {
	    ...
	    AppendMenu(m, MF_SEPARATOR, 0, 0);
		AppendMenu(m, MF_ENABLED, IDM_WINSCP, "Win SCP");
	    //if (help_path)
...
/*************************************************************
* Function                      :       ExecFunc
* Purpose                       :       Execute the command without opening a
console
* Args                          :       Pointer to the command to be executed
* Return Value          :       TRUE on success, 
*                                               FALSE otherwise
**************************************************************/
HANDLE ExecFunc(char* args)
{
	...
	rv = CreateProcess(
		...
		args,  // pointer to command line string
				...
				WPARAM wParam, LPARAM lParam)
{
	...
	switch (wParam & ~0xF) {       /* low 4 bits reserved to Windows */
	  ...
	  case IDM_WINSCP:
		...
		len = strlen(path) - (p - path);
		memmove(path, p, len);
		path[len] = 0;
		_snprintf(cmd, MAX_PATH, "C:\\Program Files\\WinSCP3\\WinSCP3.exe %s@%s:%s",
			cfg.username, cfg.host, path);
		ExecFunc(cmd);
